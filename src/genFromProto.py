...
def generate_H(classes):
    ...
    with open("generated/" + filename + ".pbs.h", 'w') as f:
        ...
        for class_name, fields in classes.items():
            ...
            field_type = ""
            field_name = ""
            members = [] #to store the names of the members of the class and their types 
            ...
            for field in fields:
                ...
                members.append((field_type, field_name, True if field[0] == "repeated" else False))
            ...
            for member in members:
                if(member[0] == "std::string"):
                    temp = "\"\";\n"
                elif(member[0] == "int32_t" or member[0] == "int64_t" or member[0] == "uint32_t" or member[0] == "uint64_t"):
                    temp = "0;\n"
                elif(member[0] == "bool"):
                    temp = "false;\n"
            ...
            for member in members:
                if(member[2] == True):
                    private_members.append("\tstd::vector<" + member[0] + "> " + member[1] + ";\n")
                    if(member[0] == "std::string"):
                        public_members.append("\tvoid add_" + member[1] + "(" + temp_string + ");\n")
                    else:
                        public_members.append("\tvoid add_" + member[1] + "(" + member[0] + ");\n")
                else:
                    private_members.append("\t" + member[0] + " " + member[1] + ";\n")
                    if(member[0] == "std::string"):
                        public_members.append("\tvoid set_" + member[1] + "(" + temp_string + ");\n")
                    else:
                        public_members.append("\tvoid set_" + member[1] + "(" + member[0] + ");\n")
            ...
            public_members.append("\tfriend std::ostream& operator<<(std::ostream& os, const " + class_name + "& " + class_name.lower()[0] + ");\n")
